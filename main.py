...
def main():
    ...
    # Initialize the components: Sense for input, Think for decision-making, Act for outputG
    sense = Sense.Sense()
    act = Act.Act()
    ...
    while cap.isOpened():
       ...
       # left_hand_gesture = sense.detect_left_hand_gesture(hand_results) if hand_results else None
        ...
        if landmarks:
            ...
            if act.mode == "cooking":
                think.update_cooking_state(elbow_angle_mvg, sense.previous_angle, shoulder_xy=shoulder, elbow_xy=elbow, wrist_xy=wrist)
            else:
                think.update_gardening_state(elbow_angle_mvg, sense.previous_angle, hip_xy=hip, wrist_xy=wrist)
        ...
        # - here I've changed the various keys we need to press
        # - you can press either the lowercase or uppercase version
        #   (so if her caps lock is accidentally on, Eleanor can still choose the right mode)
        ...
        if act.mode == "choose":
            if key in (ord('g'), ord('G')):
                act.start_countdown("gardening")
            elif key in (ord('c'), ord('C')):
                act.start_countdown("cooking")
